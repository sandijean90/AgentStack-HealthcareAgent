...
This repo shows how agents built in different frameworks can be deployed on Agent Stack. All agents run as A2A servers, so they can talk to each other whether you are running Agent Stack locally or on a hosted deployment within your organization. Imagine your org’s hosted Agent Stack where the HR agents can seamlessly call the Finance agents. This example does the same with healthcare agents. Install instructions for Agent Stack are available in the quickstart: https://Agent Stack.beeai.dev/stable/introduction/quickstart.
